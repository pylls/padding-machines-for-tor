...
def circpad_to_wf(
    trace, 
    cells=False, timecells=False, dirtime=False, cellevents=False,
    strip=False
    ):
    ...
    - cellevents, each line consists of the trace event for (non)padding cells.

    If the strip flag is set, events prior to a first domain resolution is
    stripped from the trace (if present). Circuits are typically created in the
